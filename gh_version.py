...
s3 = """Must add instruction to every mention of missed pill window.
...
Superiority claims.
“It’s about time you consider that upgrade” is a superiority claim - was changed to “it’s about time to consider Slynd”
...
with st.expander("Upload and Extract Text from Post Image(s)", expanded=True):
    ...
    # Save the caption in session state
    caption = st.text_input("Enter your caption copy here")
    if caption:
...
def check_compliance(dos_input, donts_input, onbrand_input, suzannes_rules_1, suzannes_rules_2, suzannes_rules_3, suzannes_rules_5, extracted_texts):
    ...
    Check the following text for compliance with the provided dos, don'ts, on-brand messaging, branded, IRI rules, claims, and things we cannot say.
    ...
    Format the response in Markdown, with the compliance result in bold and the supporting details bulleted and italicized. Next to each bullet, have either a ✅  or ❌, to determine if that is positive or negative feedback.
    """
